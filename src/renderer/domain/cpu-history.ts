/**
 * Pure helpers for the CPU usage graph: a fixed-capacity ring buffer of recent
 * samples and the geometry to draw and scrub them. A sample is a 0-100 percent,
 * or `null` for a tick whose reading was not OK (drawn as a gap, not a fake 0).
 */

export type CpuSample = number | null;

export const CPU_HISTORY_CAPACITY = 60;

/** Appends `sample`, keeping at most `capacity` newest entries (oldest dropped). */
export function pushSample(history: CpuSample[], sample: CpuSample, capacity = CPU_HISTORY_CAPACITY): CpuSample[] {
 ...
 */
export function sampleIndexAtFraction(fraction: number, filled: number, capacity = CPU_HISTORY_CAPACITY): number | null {
